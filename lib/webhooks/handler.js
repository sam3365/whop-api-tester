...
export function handleEvent(event) {
  ...
  switch (type) {
    case "payment.created":     return onPaymentCreated(data);
    case "payment.succeeded":   return onPaymentSucceeded(data);
    case "payment.failed":      return onPaymentFailed(data);
    case "payment.refunded":    return onPaymentRefunded(data);
    ...
    case "membership.went_invalid": return onMembershipWentInvalid(data);
    default:
...
function onPaymentCreated(data) {
  const internalMemberId = getInternalMemberId(data);
  console.log(
    `🆕 Payment created: ${data.id} — $${data.total} ${data.currency?.toUpperCase()} status:${data.status}` +
    (internalMemberId ? ` | internal_member_id: ${internalMemberId}` : ""),
  );
  // payment.created fires immediately; wait for payment.succeeded before fulfilling
}
...
function onMembershipWentInvalid(data) {
  const internalMemberId = getInternalMemberId(data);
  console.log(
    `⛔ Membership went invalid: ${data.id}` +
    (internalMemberId ? ` | internal_member_id: ${internalMemberId}` : ""),
  );
  // TODO: revoke access
}

function onPaymentRefunded(data) {
  const internalMemberId = getInternalMemberId(data);
  console.log(
    `↩️  Payment refunded: ${data.id} — $${data.refunded_amount} ${data.currency?.toUpperCase()}` +
    (internalMemberId ? ` | internal_member_id: ${internalMemberId}` : ""),
  );
  // TODO: handle refund, revoke access if applicable
}
