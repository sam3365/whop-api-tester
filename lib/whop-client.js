...
if (!process.env.WHOP_API_KEY) {
  // Warn at module load but don't throw — Next.js static analysis runs this
  // code at build time when env vars may not be present yet. The SDK will
  // throw its own error at runtime if a call is made without a valid key.
  console.warn("⚠️  WHOP_API_KEY is not set. Add it to .env.local (dev) or Vercel Environment Variables (production).");
}
 ...
 */
// Webhook signature verification is handled manually in app/api/webhooks/whop/route.js
// using Standard Webhooks HMAC-SHA256 — do not pass webhookKey to the SDK.
